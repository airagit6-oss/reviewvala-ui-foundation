...
export function ReviewValaApp() {
  ...
  const { reviews, responses, dataStatus, saveResponse, approveResponse } = useWorkspaceData();
  const content = useMemo(() => page === "Overview" ? <Overview setPage={setPage} reviews={reviews}/> : page === "Reviews" ? <ReviewsPage reviews={reviews} responses={responses} saveResponse={saveResponse}/> : page === "Response Center" ? <ResponseCenter reviews={reviews} responses={responses} approveResponse={approveResponse}/> : <ModulePage page={page}/>, [page, reviews, responses, saveResponse, approveResponse]);
